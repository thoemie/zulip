...
* If your SMTP provider uses SSL on port 465 (and not TLS on port 587), it is not enough
  to change the EMAIL_PORT in `/etc/zulip/settings.py`. Additionally, you must change delete or 
  comment out the directive "EMAIL_USE_TLS = True", and instead use a directive "EMAIL_USE_SSL = True".
